...
public class 计时器_创建计时器 : 动作
{
    ...
    public override void 渲染()
    {
        EditorGUILayout.BeginVertical();
        {
            EditorGUILayout.BeginHorizontal();
            {
                ...
                计时器类型 = EditorGUILayout.Popup("计时器类型", 计时器类型, G.计时器类型_Array, GUILayout.MaxWidth(150));
                计时周期 = EditorGUILayout.FloatField("计时周期", 计时周期, GUILayout.MaxWidth(200));
                
            }
            EditorGUILayout.EndHorizontal();
            if (GUILayout.Button("新增 计时结束触发", GUILayout.MaxWidth(150)))
            {
    ...
    private void 渲染全部行为项目()
    {
        ...
        {
            EditorGUILayout.LabelField("行为 " + i + " :");
